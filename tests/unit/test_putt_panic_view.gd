...
## players entry: [x, y, strokes, sunk, aim_x, aim_y, at_rest]
func _snapshot(players: Dictionary, bar_x := 0.0) -> Dictionary:
	return {
		"players": players,
		"cup": [PuttPanic.CUP_POS.x, PuttPanic.CUP_POS.y],
		"bar": [bar_x, PuttPanic.BAR_Y],
...
func test_setup_builds_cup_bar_and_a_ball_per_player() -> void:
	assert_not_null(view.arena.get_node("Bar"))
...
func test_sinking_pops_a_burst() -> void:
	...
	view.render(_snapshot({0: [PuttPanic.CUP_POS.x, PuttPanic.CUP_POS.y, 3, 1, 0.0, 1.0, 1]}))
	var after := 0
	for child in view.arena.get_children():
		if child is CPUParticles3D:
			after += 1
