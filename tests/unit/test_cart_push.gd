...
func test_net_pusher_advantage_moves_the_cart() -> void:
	...
	# doesn't ride on tick timing or body-separation nudges.
	...
	# Balanced 1v1 from a clean centered cart: net force is zero, so the
	# cart holds. Reset explicitly so the earlier +x drift can't leak in.
	game.cart_x = 0.0
	for slot: int in game.slots:
		_park(game, slot)
