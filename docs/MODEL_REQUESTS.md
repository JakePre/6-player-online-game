...
| MDL-007 | #785 Turbo Lap real course | Track barrier: low red-and-white striped crash barrier segment, tileable end-to-end. Chunky low-poly. | ~2u long, ~0.6u tall | `assets/generated/models/track-barrier.glb` | landed (unwired — #1112 replaced the tiled barriers with procedural curb rails; asset kept on disk for reuse) |
...
| MDL-022 | #1041 Turbo Lap (re-roll of MDL-006) | Start/finish arch RE-ROLL: the delivered `finish-arch.glb` renders as malformed geometry — a lumpy shapeless blob, not an arch (owner playtest "finish line asset still fucked"; AABB is arch-sized 8×6.45×1.78 but the triangles are garbage). Same brief as MDL-006 (checkered gold+white banner arch, two pylons, spanning the track) but it must actually read as a gate. Turbo Lap ships a procedural two-post + banner-beam gate as the fallback until this lands. Pivot at base center. | ~4.5u span, ~2.5u tall | `assets/generated/models/finish-arch.glb` | landed |
| MDL-023 | #1026 Fox character regen | Fox character regeneration — the current Fox.glb has the mesh oriented backward (77% of vertices on -Z, head has zero front-facing vertices). The concept-projection texture was projected from +Z, putting the face features on the back of the character. Fix: regenerate with the mesh facing forward (-Z forward convention) and project the concept texture from the correct forward axis. The existing GLB on disk has the correct skeleton and animations but the mesh needs to be rotated 180° on Y and the texture re-projected from the correct direction. | ~1.8u tall (KayKit skeleton) | `assets/characters/generated/Fox.glb` | requested |
| MDL-024 | #1026 Rhino character regen | Rhino character regeneration — the current Rhino.glb has the concept-projection texture projected from the wrong axis, so the face features are on the back of the head instead of the front. The mesh orientation is correct but the texture baking was from the wrong direction. Fix: regenerate with the concept texture projected from the correct forward axis (-Z). The existing GLB on disk has the correct skeleton, animations, and mesh orientation. | ~1.8u tall (KayKit skeleton) | `assets/characters/generated/Rhino.glb` | requested |
