...
| The Mole | FFA (hidden-role) | 8 | No player collision, exactly one mole regardless of headcount. `CELL_TARGET`/`MAX_LOOSE_CELLS`/wave rate are fixed regardless of crew size, so a larger crew tightens the WORK phase — an initial read recommended 12 with `MinigameScaling` economy wiring, but **owner override: capped at 8**, where the fixed economy stays close enough to the 6-player baseline that no scaling is needed. Plain bump. |
