...
- **Asset requests (`IMAGE_REQUESTS.md`/`MODEL_REQUESTS.md`) are async and owner-gated — this is by design, not a blocker.** Any agent can append a row at any time without asking; the owner batches and delivers on their own schedule, sometimes to a side branch rather than straight to `main`. That means `generated` (delivered-but-unwired) rows can sit for a while and drift out of whoever's context claimed the consuming task. When claiming a task, check both ledgers for `generated` rows that name your task/issue as consumer — landing a delivered asset (import, wire, flip to `landed`, log in `CREDITS.md`) is fair game to pick up even if you didn't file the original request. Periodically sweep for orphaned delivery branches too (`git branch -a` + `git ls-tree` against a ledger's `generated` rows) — a delivery can land on its own branch and never get merged.
